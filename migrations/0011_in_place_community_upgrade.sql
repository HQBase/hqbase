...
CREATE TABLE IF NOT EXISTS community_pro_upgrades (
  id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS community_pro_upgrades_installation_idx
ON community_pro_upgrades (installation_id, created_at DESC);

CREATE UNIQUE INDEX IF NOT EXISTS community_pro_upgrades_active_lock_idx
ON community_pro_upgrades (installation_id)
WHERE state NOT IN ('complete', 'failed', 'recovery_required');

CREATE TABLE IF NOT EXISTS community_pro_upgrade_audit (
  id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS community_pro_upgrade_audit_upgrade_idx
...
VALUES ('in_place_community_upgrade', '0011', datetime('now'))
ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at;

UPDATE pro_release_state
SET installed_schema_version = 11, updated_at = datetime('now')
WHERE singleton = 1 AND edition = 'pro';
