...
describe("HQBase Mail API draft synchronization", () => {
  ...
  it("keeps an agent draft private through create, read, update, and delete", async () => {
    const createdResponse = await apiMutation("/api/v1/drafts", agentToken, "POST", {
    ...
    const opened = await apiFetch(`/api/v1/drafts/${created.id}`, agentToken);
    ...
    const uploadedResponse = await SELF.fetch(`${origin}/api/v1/drafts/${created.id}/attachments`, {
      body: form,
    ...
    const hiddenFromPerson = await apiFetch(`/api/v1/drafts/${created.id}`, sendToken);
    expect(hiddenFromPerson.status).toBe(404);

    const updated = await apiMutation(`/api/v1/drafts/${created.id}`, agentToken, "PATCH", {
      bcc: [],
      cc: [],
    ...
    const deleted = await apiMutation(`/api/v1/drafts/${created.id}`, agentToken, "DELETE");
    expect(deleted.status, await deleted.clone().text()).toBe(204);
    expect((await apiFetch(`/api/v1/drafts/${created.id}`, agentToken)).status).toBe(404);
  });
