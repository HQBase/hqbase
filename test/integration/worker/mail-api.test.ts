...
describe("HQBase Mail API v1", () => {
  ...
  it("publishes separate connection skills, OpenAPI, and retirement notices", async () => {
    ...
    expect(humanInstructions).toContain(`- OpenAPI contract: ${origin}/api/v1/openapi.json`);
    ...
    expect(document.paths["/api/v1/messages"]?.get?.security).toContainEqual({ agentBearer: [] });
    expect(document.paths["/api/v1/messages"]?.get).toMatchObject({
      "x-hqbase-agent-capabilities": ["mail:read"]
    });
    expect(
      document.paths["/api/v1/messages/{id}/remote-media/trust"]?.post?.security
